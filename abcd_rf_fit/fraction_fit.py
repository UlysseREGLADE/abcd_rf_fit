...
def iterative_solver(
    ...
    min_rec_depth=10,
    max_rec_depth=100,
    convergance_precision=0.01,
    min_converged_passes=5,
):
    residuals = []
    ...
    for _ in range(max(min_rec_depth, min_converged_passes)):
        ...
        num, den, solution = fit_rational_fraction_polynomial_basis(
            x_norm, y, n=n, den=den
        )

        residuals.append(np.sum(np.abs(y - num / den)))

    if convergence_criteria(
        np.array(residuals), convergance_precision, min_converged_passes
    ):
...
def get_rationnal_fit(
    x, y, n, return_true_solution=True, return_residuals=False, *args, **kwargs
):
...
if __name__ == "__main__":
    ...
    y = (
        rationnal_function(x, coefs)
        + noise * np.random.normal(0, 1, (x.size))
        + noise * 1j * np.random.normal(0, 1, (x.size))
    )
    ...
    popt, pcov = complex_fit(
        fit_func, x, y, p0=np.array([*solution.real, *solution.imag], dtype=float)
    )
    ...
    plt.scatter(
        np.arange(2 * (n + 1)),
        np.abs(better_solution / better_solution[0]),
        facecolor="none",
        edgecolors="C2",
        zorder=100,
    )
